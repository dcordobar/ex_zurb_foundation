defmodule Zf.Paginator.HTML.SEO do
  @moduledoc """
  ...
  `Zf.Paginator.HTML.pagination_links/4` will use this module to add `rel` to each link produced to indicate to search engines which
  link is the `next` or `prev`ious link in the chain of links. The default is `rel` value is `canonical` otherwise.

  Additionally, it helps Google and other search engines to put `<link/>` tags in the `<head>`. The `Zf.Paginator.HTML.SEO.header_links/4`
  function requires the same arguments you passed into your `Zf.Paginator.HTML.pagination_links/4` call in the view. However, `header_links` needs
  ...
  @defaults Zf.Paginator.HTML.defaults

  @doc ~S"""
  Produces the value for a `rel` attribute in an `<a>` tag. Returns either `"next"`, `"prev"` or `"canonical"`.

      iex> Zf.Paginator.HTML.SEO.rel(%Scrivener.Page{page_number: 5}, 4)
      "prev"
      iex> Zf.Paginator.HTML.SEO.rel(%Scrivener.Page{page_number: 5}, 6)
      "next"
      iex> Zf.Paginator.HTML.SEO.rel(%Scrivener.Page{page_number: 5}, 8)
      "canonical"
  """
  ...
  Arguments are the same as `Zf.Paginator.HTML.pagination_links/4`. Consider using one of the following techniques to
  call this function: [http://blog.danielberkompas.com/2016/01/28/seo-tags-in-phoenix.html](http://blog.danielberkompas.com/2016/01/28/seo-tags-in-phoenix.html)
      ...
      iex> Phoenix.HTML.safe_to_string(Zf.Paginator.HTML.SEO.header_links(%Scrivener.Page{total_pages: 10, page_number: 3}))
  ...
  defp href(conn, paginator, args, opts, page_number) do
    ...
    path = opts[:path] || Zf.Paginator.HTML.find_path_fn(conn && paginator.entries, args)
